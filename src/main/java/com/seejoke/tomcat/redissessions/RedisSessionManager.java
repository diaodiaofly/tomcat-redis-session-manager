...
public class RedisSessionManager extends ManagerBase implements Lifecycle {
  ...
  protected String serializationStrategyClass = "com.seejoke.tomcat.redissessions.JavaSerializer";

  protected EnumSet<SessionPersistPolicy> sessionPersistPoliciesSet =
      EnumSet.of(SessionPersistPolicy.DEFAULT);
  ...
  /**
   * Start this component and implement the requirements of {@link
  ...
  protected synchronized void startInternal() throws LifecycleException {
    ...
    if (!attachedToValve) {
      String error =
          "Unable to attach to session handling valve; sessions cannot be saved after the request without the valve starting properly.";
      log.fatal(error);
  ...
  /**
   * Stop this component and implement the requirements of {@link
  ...
  public Session createSession(String requestedSessionId) {
    ...
    try {
      ...
      if (null != requestedSessionId) {
        ...
        } while (jedis.setnx(sessionId.getBytes(), NULL_SESSION)
            == 0L); // 1 = key set; 0 = key already existed
      }
